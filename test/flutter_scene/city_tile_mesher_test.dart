...
void main() {
  ...
  group('the exterior and full tiers are what they were', () {
    ...
    test('site access on the wire leaves every tier to the byte with the '
      ...
      // keeps. With it ON, R4 draws the plan: those tiers are pinned at
      // their own values below, and they are DIFFERENT, which is the whole
      // of the slice.
      ...
      // lane cut open — so every tier moves. (Track A's site mesher and
      // kerb cuts move these three again; the off values above never move.)
      for (final tier in CityTier.values) {
        expect(at(on, tier), isNot(at(knobs, tier)), reason: tier.name);
      }
      expect(at(on, CityTier.near), 0x856e8b38, reason: on.keyTerms);
