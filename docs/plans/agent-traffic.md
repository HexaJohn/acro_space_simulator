...
- **Near and far slots.**
  ...
  - A parked car's band depends on where the camera is, but the parked half is hundreds of instances this section keeps out of the shared buffer except when a car comes or goes — so the focus is held to a stride, `SiteCarPass.parkedBandStepM = 32` m: the shadow edge is fuzzy by at most 4% of itself at 800 m, and a panning camera pays the rewrite once a stride instead of once a frame.
