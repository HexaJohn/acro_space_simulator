...
- **Inside a site there is no despawn to reach it, so the wait is given up instead (from T4a).** A car at a site throat, stopped `throatStopM` inside the kerb line and asking `canJoin` for a gap each sub-step (site-access §7.4 departure steps 5–6), has element −1: the road mover skips it and nothing above runs for it. `canJoin` is a safety test with no impatient term of its own — the forced priority of §5.4 waives ETA at a stop line but never an occupied conflict, because joining into a body is a collision however long the car has waited — so a street held by something that does not move would hold the car, the throat behind it and its owner's leg for as long as the obstruction stood. The site mover therefore reads the clock §7.4 step 6 told it to keep: `throatStuckAfterS = 60` s of grace, then stuck time at the same rate, and at `stuckDespawnS` of it — **180 s** in all, on the defaults — the departure is **given up**. The car goes back on a stall of its site (the first free one in the join's order; garaged, as §7.6 row 1's last resort, when the lot filled behind it), counted in `siteStats.throatGiveUps`, and its owner's leg is planned again from the site's out-joins, exactly as §7.5's back-out give-up does it. It is a give-up and not a despawn because a car inside a site can be put back where it came from, which a car wedged on a carriageway cannot; nothing crosses the kerb to do it, so an EXIT still comes from a `canJoin` gap and from nothing else. A car queued behind it inside the site starts its own clock only when it reaches the throat. A car held at an **arrival** gate is the other way round — it is still on the carriageway, so this despawn does reach it — but reaching it there is a loss, not a resolution: the trip ends nowhere and the stall it reserved is held to the last second. It gives its stall up and takes a kerb slot ahead long before, at `gateGiveUpS` (§7.3).
...
7. **Site access (from T4a).** `sharedSingle` throats are one claim unit, with opposite claims refused and ties by (sub-step, handle) (site-access §7.4); arrival-gate forced grants and give-ups (§7.3); the home back-out's serialised claims, inbound priority and tandem shuffle (§7.5). No wait at a site is open-ended: an arrival gives its stall up after `gateGiveUpS` and takes a kerb slot ahead (§7.3), a back-out gives up after `backOutGiveUpS` (§7.5) and a throat after `throatStuckAfterS` + `stuckDespawnS` (§5.6), the last two back on a stall with the owner's leg planned again — because neither a forced grant nor `canJoin` will ever waive a body, so no refusal any of them meets is guaranteed to clear on its own.
...
1. **The destination's own stalls** (revision 4, site-access §7.4–§7.5), if the destination has a network plan current for the graph (§3.10), the arrival join is in-capable, and a stall is free.
   ...
   - The gate grants when the throat in-lane has room for the car, no outbound `sharedSingle` claim is held, a far-side left-in has its opposing gap (§5.4), and the car is at ≤ 3 m/s. A refused car is held at `destS` by its virtual leader. After 25 s the ETA part is waived (a counted forced grant); after 30 s refused — **whatever refused it** — the car releases the stall and goes to step 2.
   - **The give-up is not about the throat.** It reads the same wait the forced grant does, so no refusal is open-ended. The forced grant waives the ETA half of the opposing-gap test and never a body across the crossing, because turning in front of one is a collision however long the car has waited; a far-side left-in over a carriageway that never opens is therefore refused for ever, and used to stand at `destS` in `parkingSearch` holding a reserved stall until §5.6's road despawn took it — the trip **lost** rather than resolved, and the stall released three minutes late. The give-up ends the wait instead: the reservation goes back at once, `lotUsed` with it, and the car takes a kerb slot ahead on the lane it was already waiting in. Give-ups where the crossing was the standing refusal are counted apart, in `siteStats.gateCrossGiveUps` — a throat that fills is a lot too small, a crossing that never clears is a street too busy, and they want different answers. Nothing crosses the kerb to do any of it, so the opposing-gap rule itself is untouched: the car stops waiting on it, it is never waived.
...
- **Rendering caps:** `parkedRenderCap = 1500`, within 1.5 km of the focus (`SiteCarPass.parkedRenderCap` and `SiteCarPass.parkedRangeM`, §15.4). Two cuts, both held: the cap bounds how many are placed, the ring how far. A car outside the ring is not placed at all, so it spends no cap slot either and the cap goes to the cars near the camera; the ring's membership is read on the parked band's own stride (§13.8), not once a frame.
...
3. **Gap acceptance**, checked before the reverse starts. All of these must hold:
   ...
   | Give-up | after 300 s of waiting (`backOutGiveUpS`), the departure is abandoned: the car goes back on the stall it never left and its owner's leg is planned again from the site's out-joins, counted in `siteStats.backOutGiveUps` |
...
- **No wait is open-ended.** The forced grant waives the ETA terms and never a body in the footprint or a queue behind it, because reversing into either is a collision however long the car has waited. So a drive held by something that does not move — a van standing at the kerb, a jam that never clears, a tandem blocker nowhere will take — would otherwise hold the car, its stall and its owner's leg for as long as the obstruction stood. After `backOutGiveUpS = 300` s the car **gives the departure up**: back on its stall, counted in `backOutGiveUps`, and its owner's leg planned again from the site's out-joins (the way `remapHeld` gives up a route it cannot carry, §7.6). It may then come back by another join or the other direction, or wait out its next dwell. Nothing crosses the kerb, so "no EXIT with a body in the footprint" is untouched. This is the §5.6 principle — a wedge must not stand for ever — applied where §5.6's own despawn cannot reach: a car waiting in its stall is parked, and a parked car accrues no stuck time (site-access §7.4).
- The waits themselves are counted in **milliseconds** and saturate rather than wrap (`SiteVehicles.waitMs`, `addClock`). An `Int32` of microseconds wraps negative after 35.8 minutes, which would silently disarm the forced grant and the give-up at the very moment a wedged driveway needs them.

**Test.** `home_back_out_test` replaces site-access A9 (`home_pad_hammerhead_test`): 2 stalls; both directions on a 1+1 street; a 60 s kerb-lane stream; the inbound/outbound conflict; a tandem shuffle. It asserts no EXIT with a body in the footprint, and no deadlock over 600 s at 10× rate. Two more hold the waits themselves: an hour of agent time refused by a body in the footprint, after which the wait still reads as a wait and the forced grant still fires; and a permanent obstruction, which ends in a counted give-up at 300 s with no EXIT logged at all.
...
- **Slice 2.** The agents answer the rest, and E3a skips `roadTraffic.advance`.
  ...
  - **Reach** (`agent_reach.dart`). Bounded multi-source searches over directed edges. Revision 5 (474c7aa): the searches do not use the path budget's skim lane, which §4.8 serves only when the queue is empty, so a busy colony would never publish. They are also never driven by a question. The noise, land-value and reach pass runs in the sub-step, after `stats.epoch`, on its own `AgentTuning.readoutWorkPerStep`. That budget was 24,000 units and was lowered to 8,000 in 21bf8b9: at 24,000 the 25× held frame broke the 3.2 ms gate at p99 on the 2-mile sprawl. At 8,000 every frame passes, and a pass publishes within 3 pictures (6 s); a whole pass there costs about 190k units at 26–31 ns each (readout_pass_bench_test). It starts at a picture and publishes only at a picture, and a lane-graph change drops it. Every readout query is a pure read. `readout_determinism_test` pins it: asking every question between ticks, or none, leaves the digests and answers identical. Reach covers: `serviceReach` from every station that sends vehicles, `fireReach` from stations with safety cover only, and `deliveryReach` from every goods source but the lot's own (D47). The radius is the routed model's, `TrafficTuning.serviceReachM` (4 km).
...
- **Near and far slots.**
  ...
  - **Every band and range is measured in anchor-relative METRES**, as the knobs are (§15.4). An instance matrix's translation is NOT: `TrafficRoad.writePose` puts the centre through `lengthToScene` on its way in, so `m.storage[12..14]` is the scene's kilometres. T4a's site and parked placement banded off the matrix and so compared kilometres with metres — every distance came out 1,000× too small, so every site and parked car read as near, always cast, and was never range-culled. `SiteCarPass.writePose` now hands its caller that centre in metres (a required `centreM` out-parameter) and the band is measured from it; `agent_traffic_pass_test` pins the unit at the two edges (800 m and 3,500 m).
  - A parked car's band depends on where the camera is, and so does its place inside or outside `parkedRangeM`'s ring (§7.4), but the parked half is hundreds of instances this section keeps out of the shared buffer except when a car comes or goes — so the focus is held to a stride, `SiteCarPass.parkedBandStepM = 32` m: the shadow edge is fuzzy by at most 4% of itself at 800 m, the ring's edge by 2% of itself, and a panning camera pays the rewrite once a stride instead of once a frame. The ring is therefore evaluated exactly when that stride lets a placement through, and costs no frame anything the band did not already cost.
...
- **Caps.** `agentRenderCap` (a static knob, default 1500) and `agentRangeM` 3500. Pedestrians and parked cars have their own caps, and the parked half its own ring — `parkedRenderCap` 1500 within `parkedRangeM` 1500 m (§7.4, §8.4). Every one placed is an instance the frame BINDS, which the upload gate above does not reach: the host buffer's storage is cycled every frame, so a bound pack is uploaded whether or not a matrix in it moved.
...
- **Parked cars:**
  - in a lot, from T4a: `[ownerIdx, where = 0, siteIdx, stallKey, variant]`, i.e. by `(siteId, stallKey)`, **never by stall index** (C-19). `siteIdx` names the lot or site id string current at save (renames follow `_carryRenamedLots`). On load, after the site-access full drain (site-access §4.4), the stall is `stallIndexOfKey(stallKey)` on the site's plan; if the key is gone, the nearest free stall by distance; if there is none, the car is garaged. A site unknown after load drops its lot cars. Reservations and vehicles inside sites are not saved. **The drain is not a guarantee** — it is complete when `CitySim.fromJson` returns, and the cars go down at the first prime with a lane graph, which is a tick or more later and after a budgeted sync of its own: a lot whose row has not arrived by then is HELD rather than garaged (the "As built" note under slice T4a);
...
- **Open.**
  - **Mesh cost, measured** (lane_ribbon_bench_test, JIT). On the 2-mile sprawl there is one overlay line per lane: 2,109 lines, 117k vertices, 6.7 MB.
    - The overlay's own `lines()` rebuild costs under 0.1 ms, but `RoadOverlayMesher.build` takes a median of 12 ms (worst 24–36 ms) on every band change, up to every 2 s while the view is open.
...
- **Order.**
  ...
  - P2: the gate, forward-out departures, the home back-out (the footprint claim, tandem LIFO and the shuffle), and access events.
...
- **Scope (per site-access §9).**
  - Cars on CommuteSynth trips only: no trucks in bays, no pedestrians.
  - Those trips drive in and park at homes too, and back out of the home pad stalls. So the home back-out and A9 are T4a's.
  - T4b adds residents' own cars living at home pads and kerbs (E36 completes), pedestrians, and full D17 circling and give-up.
...
- **The renderer, after the road side measured a grown town pushing 4.0 MB of instance transforms a frame at 95 m.**
  ...
  - **§7.4's ring, landed:** a parked car further than `SiteCarPass.parkedRangeM` (1,500 m, a knob beside the cap, §15.4) from the focus is not placed at all — not bound, not uploaded, and no cap slot spent on it, so `parkedRenderCap` goes to the cars near the camera. The cap and the ring are separate cuts and both hold: a dense town under the camera still caps at 1,500, a sparse one spread over a plat still culls. The distance comes from `SiteCarPass.centreOf` before a pose is worked out at all, so a car the ring drops costs neither the basis nor the matrix, and membership is read on the band's own 32 m stride — the ring changes with the focus exactly as the shadow band does, and re-reading it every frame would give back what `0495be9` bought. Measured on the generator's sprawl at a 95 m camera, with a parked car on every lot: the 2-mile town's 1,264 parked cars were 1,264 placed and 1,344 instances bound a frame (84 KiB of transforms) and are now 850 and 896 (56 KiB); the 12-mile town's 1,500 — the cap, spent anywhere out to 14 km — are now 934, every one of them inside the ring. A town whose parking exceeds the cap keeps binding 1,500 either way; what the ring buys there is that all 1,500 stand near the camera. `parkedRangeM = infinity` puts T4a's behaviour back for an A/B, and the test that asserted the 9 km car IS drawn now asserts the ring drops it.
- **A pre-slice-3 save, captured before the owners move.** `test/persistence/fixtures/agents_v2_t4a_pre_citizens.json` is a real `CitySim.toJson()` of the grown dev colony taken from this build (2026-09-17, `64d6604`): a `v: 2` block of 67 parked cars over 58 sites — 56 on stalls, 5 at the kerb, 6 garaged, on home driveways, car parks, installation yards and `kerbOnly` sites, 11 of them on lots a re-cut renamed before the save. `test/persistence/agents_v2_pre_citizens_fixture_test.dart` loads it and pins what slice 3's owner port must not break: every lot car back on the stall its `(siteId, stallKey)` names, `where`/`kind`/`variant` per car, the counts by `where`, and the garaged-not-dropped rule. It must NEVER be regenerated — a round trip inside one build writes and reads the new meaning and passes while a player's older save quietly re-owns its cars — so its generator is skipped unless asked for by name (`--dart-define=ACRO_WRITE_AGENTS_FIXTURE=true`).
- **A load holds what its sites have not caught up with (§14.1).** Found while capturing the fixture above: `_placeSavedCars` ran ONCE, at the first prime with a lane graph, and `takeSaved()` was consumed there, so a site the colony had not planned by that instant read exactly as a site that is gone — `rowOfSite` answers −1 for both — and its cars were garaged (or dropped) with no second chance. The book plans on a budget and a road edit leaves the 127k-site stand-in re-planning for some 260 ticks (§7.6 as built), so a colony loading with a backlog in flight quietly lost lot cars; the capture had to settle 60 s after its re-cut to avoid saving one short. **The drain in `CitySim.fromJson` guarantees only that the book is complete when `fromJson` RETURNS**: the cars go down a tick or more later, after `CitySim.advance` has grown the colony and re-synced the book under the per-tick budget, and after any road edit a host makes between the two. The rule as built:
  - a saved **lot** car whose site has no row, but whose building stands, is offered to the world through `CarRestoreSink.holdForSite` and HELD — placed nowhere, not garaged. Only a lot car waits: a kerb car hangs on no site row, and a garaged car is garaged whenever it is asked.
  - A car whose building is gone is dropped as before, and a car whose row IS there is placed as before (the key, the nearest free stall, the garage).
  - Held cars are retried when the **site table syncs** — a `sitesRev` move, a chunk move, a new graph — never on a timer. A retry is `AgentsCodec.restoreCars` again over the held rows alone, so the order and the answers are the ones one pass would have given.
  - **The bound.** The waiting ends at the first prime where either `SitePlanSource.plansComplete` is true — `BookPlanSource` reads the book's own `lastSync.complete`, so a colony loaded through `fromJson` normally settles at its first or second tick — or `kRestoreHoldS` (300 agent seconds) has passed since the load, whichever comes first. The time bound is the backstop for a source that can never say it is finished; 300 s is over twice the ~260 ticks a road edit costs, and the same five minutes a home back-out waits. Whatever is still held is then resolved exactly as this build resolved it at the first prime: garaged at its building. The bound is asked at **every** prime, not only at a sync, so a colony whose plans never move again is never left holding.
  - **Counted** in `SiteStats`, and folded into the digest: `restoreHeld`, `restoreGaveUp`, `restoreDropped`. A quiet load reads 0, 0, 0.
  - **A save taken while cars are held writes them back out.** `AgentsCodec.encode` takes the held block and the rows of it still waiting and emits them beside the cars that are down, in the same two row shapes — the format does NOT change, and the road side's pin and the fixture hold. So does a save taken before the load's first advance, which used to write `{"v": 1}` and lose every car in the block it was still holding. A save, a load and a save again carry the same cars whatever the colony managed to place in between, and no save's cars are ever left in limbo across the next one.
  - `test/traffic/restore_waits_for_sites_test.dart` drives it: a load whose plan source withholds half the parked sites (41 of 82 cars were garaged for good before this), the two bounds, the drop, and the lossless save-load-save.
