...
7. **Site access (from T4a).** `sharedSingle` throats are one claim unit, with opposite claims refused and ties by (sub-step, handle) (site-access §7.4); arrival-gate forced grants and give-ups (§7.3); the home back-out's serialised claims, inbound priority and tandem shuffle (§7.5). No wait inside a site is open-ended: a back-out gives up after `backOutGiveUpS` (§7.5) and a throat after `throatStuckAfterS` + `stuckDespawnS` (§5.6), both back on a stall with the owner's leg planned again, because neither the forced grant nor `canJoin` will ever waive a body.
