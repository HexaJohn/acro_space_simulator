...
- `lib/src/scene.dart` — `Scene.collectorPacerBytesPerFrame` (1 MiB of
  16 KiB chunks, 0 to disable): Uint8Lists allocated and dropped at the end
  of every frame so the collector's minor collections run every couple of
  dozen frames instead of every seventy. The per-frame command buffers and render passes have no dispose
